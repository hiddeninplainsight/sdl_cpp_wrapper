...
namespace sdl
{
    rectangle_collider::rectangle_collider(int width, int height, anchor_point anchor)
    {
        ...
        case anchor_point::top_right:
            translate_points(collider_shape, collider_shape, number_of_collider_points, -width, 0);
            break;
        case anchor_point::bottom_left:
            translate_points(collider_shape, collider_shape, number_of_collider_points, 0, -height);
            break;
        case anchor_point::bottom_right:
            translate_points(collider_shape, collider_shape, number_of_collider_points, -width, -height);
            break;
        case anchor_point::center:
            translate_points(collider_shape, collider_shape, number_of_collider_points, -(width / 2), -(height / 2));
            break;
        default:
    ...
    void rectangle_collider::set_translation(SDL_Point translation, bool recalculate)
    {
    ...
    {
        scale_points(collider_shape, scaled_collider, number_of_collider_points, scale_transform);
        rotate_points(scaled_collider, rotated_collider, number_of_collider_points, rotate_transform);
        translate_points(rotated_collider, translated_collider, number_of_collider_points, translate_x_transform, translate_y_transform);
