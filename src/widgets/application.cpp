...
namespace sdl
{
	namespace widgets
	{
		...
		application::~application() { current = nullptr; }
		void application::add_widget(widget* w) { widgets.push_back(w); }
		void application::remove_widget(widget* w)
		{
			widgets.erase(std::remove(widgets.begin(), widgets.end(), w),
						  widgets.end());
		}
