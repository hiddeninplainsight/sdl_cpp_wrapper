...
namespace sdl
{
	...
	void events::poll() const
	{
		...
		while (SDL_PollEvent(&event))
		{
			...
			case SDL_KEYDOWN:
				...
				{
					key_down_handler->key_down_event(event.key.keysym.sym,
			...
			case SDL_KEYUP:
				...
				{
					key_up_handler->key_up_event(event.key.keysym.sym,
												 event.key.keysym.mod);
				}
				break;
			case SDL_MOUSEMOTION:
				...
				{
					mouse_motion_handler->mouse_motion_event(
						event.motion.which, event.motion.state, event.motion.x,
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				{
					mouse_button_handler->mouse_button_pressed_event(
						event.button.which,
			...
			case SDL_MOUSEBUTTONUP:
				if (mouse_button_handler)
				{
					mouse_button_handler->mouse_button_released_event(
						event.button.which,
						SDL_BUTTON_to_mouse_button(event.button.button),
						event.button.clicks, event.button.x, event.button.y);
				}
			...
			case SDL_MOUSEWHEEL:
				...
				{
					mouse_wheel_handler->mouse_wheel_event(
	...
	void events::clear_quit_event_handler() { quit_handler = nullptr; }
	void events::set_key_down_event_handler(key_down_event_handler& handler)
	{
		key_down_handler = &handler;
	}

	void events::clear_key_down_event_handler() { key_down_handler = nullptr; }
	void events::set_key_up_event_handler(key_up_event_handler& handler)
	{
		key_up_handler = &handler;
	}

	void events::clear_key_up_event_handler() { key_up_handler = nullptr; }
	void events::set_mouse_motion_event_handler(
	...
	void events::set_mouse_button_event_handler(
	...
	void events::set_mouse_wheel_event_handler(
