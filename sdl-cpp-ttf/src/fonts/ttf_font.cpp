...
namespace sdl
{
	namespace fonts
	{
		...
		{
			font =
				TTF_OpenFontRW(SDL_RWFromConstMem(memory, size), 1, point_size);
			if (font == nullptr)
			{
		...
		ttf_font::ttf_font(ttf_font&& other) noexcept : font{other.font}
		{
			other.font = nullptr;
		}
		...
		surface ttf_font::create_surface(char const* text,
										 SDL_Color colour) const
		{
			...
			{
				throw sdl_exception{"fonts::create_surface", TTF_GetError()};
			}

			return surface{text_surface};
		}

		surface ttf_font::create_surface(std::string const& text,
										 SDL_Color colour) const
		{
			return create_surface(text.c_str(), colour);
		}

		texture ttf_font::create_texture(const renderer& renderer,
										 char const* text,
										 SDL_Color colour) const
		{
