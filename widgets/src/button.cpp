...
namespace sdl
{
	namespace widgets
	{
			...
			, outline_points{{0, 0}, {100, 0}, {100, 100}, {0, 100}, {0, 0}}
		{
			...
			current_application()->mouse_events.add_mouse_button_event_handler(
				*this);
		}
		...
		std::string const& button::text() const { return text_widget.text(); }
		void button::text(std::string const& text) { text_widget.text(text); }
		void button::draw()
		{
		...
		void button::set_outline_points()
		{
			...
			translate_points(outline_points, outline_points,
							 number_of_outline_points, location.x, location.y);
		}

		void button::mouse_button_pressed_event(Uint32 mouse_id,
												mouse_button button,
												...
												Sint32 y)
		{
			...
			if (is_visible && button == mouse_button::left)
			{
				...
				bool hit =
					tri_point_collision(&outline_points[0], mouse_point) ||
		...
		void button::mouse_button_released_event(Uint32 mouse_id,
												 mouse_button button,
												 Uint8 clicks, Sint32 x,
												 Sint32 y)
		{
